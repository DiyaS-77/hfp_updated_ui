...
from connect_profile_api_added_host_ui import TestApplication
...
from Utils.utils import start_obexd_daemon
from Utils.utils import start_ofonod_daemon
from Utils.utils import start_pulseaudio_daemon
from Utils.utils import stop_bluetoothd_daemon
from Utils.utils import stop_daemons
from Utils.utils import stop_dump_logs
from Utils.utils import stop_obexd_daemon
from Utils.utils import stop_ofonod_daemon
...
class BluetoothUIApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.controllers_list_layout = None
        self.controllers_list_widget = None
        self.test_app_instance = None
        self.bluetoothd_log_file_path = None
        self.pulseaudio_log_file_path = None
        self.obexd_log_file_path = None
        self.ofonod_log_file_path = None
    ...
    def list_controllers(self):
        ...
        self.controllers_list_widget.setStyleSheet(styles.controllers_list_widget_style_sheet)
    ...
    def controller_selected(self, address):
        ...
        self.hcidump_log_name = start_dump_logs(self.interface, self.log, self.log.log_path)
    ...
    def test_application_clicked(self):
        ...
        if not hasattr(self, 'daemons_started'):
            ...
            self.bluetoothd_log_file_path = start_bluetooth_daemon(log=self.log)
            self.pulseaudio_log_file_path = start_pulseaudio_daemon(log=self.log)
            self.obexd_log_file_path = start_obexd_daemon(log=self.log)
            self.ofonod_log_file_path = start_ofonod_daemon(log=self.log)
            self.daemons_started = True

        self.setWindowTitle('Test Host')
        self.test_app_instance = TestApplication(interface=self.interface, back_callback=self.show_main, log=self.log,
                                                 bluetoothd_log_file_path=self.bluetoothd_log_file_path,
                                                 pulseaudio_log_file_path=self.pulseaudio_log_file_path,
                                                 obexd_log_file_path=self.obexd_log_file_path,
                                                 ofonod_log_file_path=self.ofonod_log_file_path,
                                                 hcidump_log_name=self.hcidump_log_name)
        self.setCentralWidget(self.test_app_instance)
...
if __name__ == "__main__":
    ...
    def stop_logs():
        """Stops hcidump logging processes and all related bluetooth daemons before application quit."""
        stop_dump_logs(app_window.log)
        stop_bluetoothd_daemon(app_window.log)
        stop_pulseaudio_daemon(app_window.log)
        stop_obexd_daemon(app_window.log)
        stop_ofonod_daemon(app_window.log)
